...
def showCorrectMenu(orderValsArr, orderWidgetsArr,tempIDs,a,b,c):
    if orderValsArr[3].get()=="":
        ...
        agvLabel.grid(after=conditionMenu)
        agvMenu.grid(after=agvLabel)
        agv.set(agvOptions[0])
        partTypeLabel.grid(after=agvMenu)
        partTypeMenu.grid(after=partTypeLabel)
        partColorLabel.grid(after=partTypeMenu)
        partColorMenu.grid(after=partColorLabel)
        ...
        annIDLabel.grid(after=conditionMenu)
        annIDMenu.grid(after=annIDLabel)
        annID.set(tempIDs[0])
        time.set('')
        agv.set('')
        partType.set('')
